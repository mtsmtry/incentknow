...
import Incentknow.AppM (class Behaviour, getAccount, navigate)
import Incentknow.Atoms.Icon (remoteWith)
import Incentknow.Atoms.Inputs (menuPositiveButton)
import Incentknow.Data.Entities (FocusedSpace, RelatedSpace, IntactAccount)
...
type State
  = { publishedSpaces :: Remote (Array RelatedSpace)
    , followedSpaces :: Remote (Array RelatedSpace)
...
initialState input = { publishedSpaces: Loading, followedSpaces: Loading, account: Nothing }

render :: forall m. MonadEffect m => Behaviour m => State -> H.ComponentHTML Action ChildSlots m
render state =
  centerLayout { css: "page-spaces", leftSide: [], rightSide: [] }
    [ maybeElem state.account \_ ->
        HH.div [ css "page-spaces-part" ]
          [ HH.div [ css "caption" ]
              [ HH.text "My spaces"
...
handleAction = case _ of
  Initialize -> do
    ...
    for_ account \_->
      callbackQuery FetchedFollowedSpaces $ getMySpaces unit
  HandleInput _ -> do
