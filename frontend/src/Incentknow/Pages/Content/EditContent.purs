...
import Incentknow.Api (Content, Work, commitContent, getContent, onSnapshotWork, writeContentWork)
import Incentknow.Api.Utils (Fetch(..), Remote(..), executeApi, fetchApi, forFetch, subscribeApi, toMaybe)
    ...
    case maybeWork of
      Just work -> do
        ...
        when (state.saveState == HasNotChange || state.saveState == Saved) do
          H.modify_ _ { value = work.data }
      Nothing -> 
        -- コンテンツをコピーする
        H.modify_ \state -> case toMaybe state.data of
          Just content -> state { value = content.data }
          Nothing -> state
  Load -> do
    state <- H.get
    fetchApi FetchedContent $ getContent state.contentId
  FetchedContent fetch ->
    case fetch of
      FromCache content -> do
        H.modify_ _ { data = Holding content }
      FromServer content -> do
        state <- H.modify _ { data = Holding content }
        workSubId <- subscribeApi (N.toMaybe >>> ChangeWork) $ onSnapshotWork (unwrap state.contentId)
        H.modify_ _ { workSubId = Just workSubId, disabled = false }
      _ -> pure unit
